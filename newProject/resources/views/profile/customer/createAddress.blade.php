...
<div class="container mx-auto px-4 mt-6 flex flex-col md:flex-row gap-6">
    ...
    <div class="w-full md:w-1/4 bg-white rounded-lg shadow sticky top-5 h-fit">
        ...
        <ul class="p-4 space-y-2 text-sm">
            ...
            <a href="{{ route('profile.customer.address.index') }}" class="flex items-center py-2 px-3 bg-purple-50 text-purple-600 font-semibold rounded-md transition">
    ...
    <div class="w-full md:w-3/4">
        <h2 class="text-2xl font-bold mb-4">เพิ่มที่อยู่ใหม่</h2>
        <form action="{{ route('profile.customer.address.store') }}" method="POST">
            @csrf
            @include('profile.customer.form')
            <button type="submit" class="mt-4 bg-purple-600 text-white px-4 py-2 rounded hover:bg-purple-700">บันทึก</button>
        </form>
    </div>
</div>
@endsection
